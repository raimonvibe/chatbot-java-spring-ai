...
2. **Widget script** is loaded from the backend (`/js/chatbot-widget.js`) on the customer’s site. It calls:
   - `GET /api/chat/embed/{embedCode}` — public, returns non-sensitive config (name, description, sanitized branding).
   - `POST /api/chat/embed/{embedCode}` — public, sends messages; rate-limited and validated.
...
| **Abuse / DoS** | (1) **Global**: `/api/chat/*` is rate-limited per client (IP or token) via `RateLimitingFilter` (e.g. 20 req/min). (2) **Per-IP per-chatbot**: POST `/api/chat/embed/{embedCode}` enforces 30 messages per hour per IP per chatbot so one visitor cannot exhaust the owner’s quota. (3) GET `/api/chat/embed/{embedCode}` rejects invalid/unsafe embedCode values and also caps path length &gt; 255 to prevent DoS. (4) Chat request body is validated (message 1–2000 chars, pattern; sessionId and language format). |
| **Sensitive data leakage** | GET `/api/chat/embed/{embedCode}` returns only: `name`, `description` (capped), `primaryLanguage`, `supportedLanguages`, `brandingConfig` (sanitized), and optional `avatar` (validated `1`–`6`). No tokens, API keys, or owner PII. |
