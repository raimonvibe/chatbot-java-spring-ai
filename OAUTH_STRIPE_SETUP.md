...
3. Test the subscription flow:
   ...
   curl -X POST http://localhost:8081/api/subscription/create-checkout-session \
   ...
   # Cancel subscription
   curl -X POST http://localhost:8081/api/subscription/cancel \
     -H "Authorization: Bearer YOUR_JWT_TOKEN"
   ```
...
1. Try accessing chatbot endpoints without a subscription:
   ```bash
   curl http://localhost:8081/api/chatbots \
     -H "Authorization: Bearer YOUR_JWT_TOKEN"
   ```
