...
public class ChatbotController {
        ...
        try {
            ...
            try {
                websiteAnalysisService.analyzeWebsite(savedChatbot)
                    .thenAccept(extractedContent -> {
                        // Automatically index content after analysis completes
                        logger.info("Website analysis completed for chatbot {}. Starting automatic indexing...", savedChatbot.getId());
                        try {
                            aiChatbotService.indexWebsiteContent(savedChatbot);
                            logger.info("Automatic indexing completed for chatbot {}", savedChatbot.getId());
                        } catch (Exception indexError) {
                            logger.error("Failed to auto-index content for chatbot {}: {}",
                                savedChatbot.getId(), indexError.getMessage(), indexError);
                        }
                    })
                    .exceptionally(analysisError -> {
                        logger.error("Website analysis failed for chatbot {}: {}",
                            savedChatbot.getId(), analysisError.getMessage(), analysisError);
                        return null;
                    });
            } catch (Exception e) {
    ...
    public ResponseEntity<Map<String, Object>> analyzeWebsite(@PathVariable Long id,
        ...
        try {
            ...
            // Start website analysis asynchronously with automatic indexing
            websiteAnalysisService.analyzeWebsite(chatbot)
                .thenAccept(extractedContent -> {
                    // Automatically index content after analysis completes
                    logger.info("Website analysis completed for chatbot {}. Starting automatic indexing...", chatbot.getId());
                    try {
                        aiChatbotService.indexWebsiteContent(chatbot);
                        logger.info("Automatic indexing completed for chatbot {}", chatbot.getId());
                    } catch (Exception indexError) {
                        logger.error("Failed to auto-index content for chatbot {}: {}",
                            chatbot.getId(), indexError.getMessage(), indexError);
                    }
                })
                .exceptionally(analysisError -> {
                    logger.error("Website analysis failed for chatbot {}: {}",
                        chatbot.getId(), analysisError.getMessage(), analysisError);
                    return null;
                });
            ...
            Map<String, Object> response = Map.of(
                ...
                "estimatedPages", estimatedPages,
                "message", "Website analysis started. Content will be automatically indexed when complete."
            );
